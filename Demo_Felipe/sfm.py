...
Class containing the main reconstruction loop.
...
class SFM:
    """
    Incremental SfM reconstructer.
    
    Args:
        views: List of view class objects
        matches: List of match class objects
    ...
    def remove_mapped_points(self, match_object, image_idx):
        ...
        Change is made within the match classes' objects.
        
        Args:
            match_object: Match class object of views
            image_idx: Index of image to be analyzed
    ...
    def compute_pose(self, view1, view2=None, is_baseline=False):
        ...
        Args:
            view1: View to be computed
            view2: Second view in case baseline must be computed, default = None
            is_baseline: Bool stating if the view is baseline, default = False
        """
        ...
        # procedure for estimating the pose of all other views
            ...
            logging.info(f"Computing pose of new image {view1.name}")
            ...
            for i, old_view in enumerate(self.done):
                ...
                _ = utils.compute_F(old_view, view1, match_object) # Remove outliers
    ...
    def triangulate(self, view1, view2):
        ...
        Also updates the point_map dictionary 
        
        Args:
            view1: First view class
            view2: Second view class
            
        """

        K_inv = np.linalg.inv(self.K)
        P1 = np.hstack((view1.R, view1.t)) # Projection matrices
        P2 = np.hstack((view2.R, view2.t))

        match_object = self.matches[(view1.name, view2.name)]
        pixel_points1, pixel_points2 = utils.get_keypoints_from_indices(keypoints1=view1.keypoints,
    ...
    def compute_pose_PNP(self, view):
        ...
        Args: 
            views: List of all views class object
    ...
    def plot_points(self):
        ...
        logging.info('Saved at %s', filename)
    ...
    def reconstruct(self):
        ...
        logging.info(f"Computing pose of baseline {baseline_view1.name} and {baseline_view2.name}")
        ...
        self.plot_points()
        logging.info("Points plotted for %f views\n", len(self.done))
        
        for i in range(2, len(self.views)):

            logging.info("Computing pose and reconstructing points for view %s", self.views[i].name)
            self.compute_pose(view1=self.views[i])
            logging.info("Mean reprojection error for %d images is %f", i+1, self.errors[i])
            self.plot_points()
            logging.info("Points plotted for %d views\n", i+1)
