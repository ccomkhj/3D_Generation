...
Class containing the baseline for Incremental SfM.
...
class Baseline:
    """
    Class containing functions that compute the baseline pose from the initial images of a reconstruction
    
    Args: 
        view1: First image class 
        view2: Second image class
        match_object: Match object of corresponding images
    ...
    def get_pose(self, K):
        ...
        F = utils.compute_F(self.view1, self.view2, self.match_object)
    ...
    def check_pose(self, E, K):
        """
        Retrieves the rotation and translation components from the essential matrix by 
        decomposing it and verifying the validity of the 4 possible solutions
        ...
        R1, R2, t1, t2 = utils.get_camera_from_E(E)  # decompose E
        if not utils.check_determinant(R1):
            R1, R2, t1, t2 = utils.get_camera_from_E(-E)  # change sign of E if R1 fails the determinant test
    ...
    def triangulate(self, K, R, t):
        """'
        Triangulate points between the baseline views and calculates the mean reprojection error 
        ...
        Returns:
            ...
            points_3D: 3D coordinates of point matches
        """
        ...
        pixel_points1, pixel_points2 = utils.get_keypoints_from_indices(keypoints1=self.view1.keypoints,
        ...
        error = np.mean(reprojection_error)
        
        return error, points_3D