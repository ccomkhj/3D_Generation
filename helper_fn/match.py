...
class Match:
    ...
    def get_matches(self, view1, view2):
        ...
        ''' Visualization of the first matching only '''
        #-- Draw matches
        import numpy as np
        img1 = cv2.imread('images/1_left.jpg')
        img2 = cv2.imread('images/2_center.jpg')
        img_matches = np.empty((max(img1.shape[0], img2.shape[0]), img1.shape[1]+img2.shape[1], 3), dtype=np.uint8)
        cv2.drawMatches(img1, view1.keypoints, img2, view2.keypoints, matches, img_matches)
